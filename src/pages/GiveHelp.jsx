...
import { CATEGORIES, URGENCY_LEVELS, POST_STATUS, STATUS_LABELS } from '../lib/constants'
...
export default function GiveHelp() {
  ...
  useEffect(() => {
    if (selectedId && listRefs.current[selectedId]) {
      listRefs.current[selectedId].scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
  }, [selectedId, posts])
  ...
  const coveredCount = posts.filter((p) => p.status === POST_STATUS.COVERED).length
      ...
      <div className="give-help-layout">
        ...
        <div className="give-help-list">
          ...
          {!loading && filtered.length === 0 && (
            <p className="empty-state">No hay publicaciones que coincidan con estos filtros.</p>
          )}
          {filtered.map((post) => {
            ...
            return (
              ...
              >
                ...
                <div className="action-rail">
                  <HelperPingButton postId={post.id} helperPings={post.helperPings} />
                  <ShareButton
                    post={post}
